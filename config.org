...
(setq jabber-sort-order (quote ("" "away" "dnd" "xa")))
*** Logging settings
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (let ((local-conf-name (expand-file-name "current-host.el" (expand-file-name "perhost" dotfiles-dir)))
        (perhost-conf-name (expand-file-name (expand-file-name (format "%s.el" (first (split-string system-name "\\."))) "perhost") dotfiles-dir)))
    ...
    (cond ((file-exists-p perhost-conf-name)
          ...
          ((message "Perhost config %s doesn't exist" perhost-conf-name))))
